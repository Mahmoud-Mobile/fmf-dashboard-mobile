...
import { fetchGuests, fetchGuestById } from "../../redux/actions/api";
...
const ArrivalFlights = () => {
  ...
  const handleTestAPI = () => {
    ...
    dispatch(fetchGuests(testParams));
  };

  const renderGuestCard = ({ item }) => (
    <GuestCard guest={item} onPress={handleGuestPress} />
  );
  ...
  const filterGuests = () => {
    return guests.filter((guest) => {
      // First filter by arrival status
      const isArrivalGuest = guest.isGuestArrived || guest.arrivalDate;

      // Then filter by search query
      if (!searchQuery.trim()) return isArrivalGuest;
